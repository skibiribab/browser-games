...
puzzle-arcade is a full-stack monorepo. Adding a game follows a fixed pattern; the game logic itself is tracked as issues.

## Steps

1. **Catalog** — add/update the entry in `catalog/games.yaml` (title, slug, `/play/<slug>` route, status, tags).
2. **Frontend app** — a React + Vite app under `frontend/apps/<slug>/`, scoped `@puzzle-arcade/<slug>`. Copy an existing app shell as the starting boilerplate (Vite + tailwind + tsconfig).
